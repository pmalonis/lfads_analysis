...
if __name__=='__main__':
    ...
    for dset_idx, (dataset, param) in enumerate(zip(datasets, params)):
        ...
        X,y = opt.get_inputs_to_model(peak_df, co, trial_len, dt, df=df, use_rates=True, rate_pcs=n_co,
                                    win_start=win_start, 
                                    win_stop=win_stop)

        fb_X,fb_y = opt.get_inputs_to_model(fb_peak_df, co, trial_len, dt, df=df, use_rates=True, rate_pcs=n_co,
    ...
    plt.savefig('../figures/final_figures/rate_averages_correlation_with_maxima_means_all.png')