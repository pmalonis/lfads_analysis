...
if __name__=='__main__':
    ...
    for i, (dataset, param) in enumerate(zip(datasets, params)):
        ...
        fb_peak_df_train = pd.read_pickle('../data/peaks/%s_%s_fb_peaks_train.p'%(dataset,param))
        fb_peak_df_test = pd.read_pickle('../data/peaks/%s_%s_fb_peaks_test.p'%(dataset,param))
        ...
        plt.figure()
        plt.scatter(co_mean, fb_co_mean)
        plt.xlabel('Initial Control--Direction Means')
        plt.ylabel('Corrective Control--Direction Means')
        plt.figure()
        plt.scatter(co_max, fb_co_max)

        print("Mean Average Correlation: %f"%np.corrcoef(co_mean, fb_co_mean)[1,0])
        print("Max Average Correlation: %f"%np.corrcoef(co_max, fb_co_max)[1,0])