...
def plot_average(event_name, win_start, win_stop, event_label,
    ...
    for dset_idx, (dataset, param) in enumerate(zip(datasets, params)):
        ...
        n_co = co.shape[2]
        peak_df_train = pd.read_pickle(os.path.join(os.path.dirname(__file__), 
                            '../data/peaks/%s_%s_train.p'%(dataset, event_name)))
        peak_df_test = pd.read_pickle(os.path.join(os.path.dirname(__file__), 
                            '../data/peaks/%s_%s_test.p'%(dataset, event_name)))
