...
params = ['final-fixed-2OLS24']#, 'mack-kl-co-sweep-0Wo8i9']
nbins = 12
    ...
    for i, (dataset, param) in enumerate(zip(datasets, params)):
        ...
        for j in range(n_co):
            ...
            for i in range(-nbins//2, nbins//2):
                ...
                co_av[j*win_size:(j+1)*win_size, i+nbins//2] = X[:,j*win_size:(j+1)*win_size][(theta > min_theta) & (theta <= max_theta)].mean(0)
