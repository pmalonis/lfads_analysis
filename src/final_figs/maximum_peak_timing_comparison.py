...
if __name__=='__main__':
    ...
    for dset_idx, dataset in enumerate(datasets):
        ...
        for event_idx, event in enumerate(events):
            latencies = [peak_df[dataset]['%s_latency_%d'%(event, i+1)].values for i in range(all_n_inputs[dset_idx])]
            ...
            print("%s, %s: %d"%(dataset, event, len(latencies_notnull)))
            if 'target' in event:
                #sns.distplot(latencies_notnull, hist=False, color=colors[event_idx])
                sns.distplot(latencies_notnull, hist=False, color=colors[event_idx], kde_kws={'gridsize':1000})
