...
win_stop = 0.2#0.1#cfg['post_target_win_stop']
...
if __name__=='__main__':
    ...
    for i, (dataset, param) in enumerate(zip(datasets, params)):
        ...
        X,y = opt.get_inputs_to_model(peak_df, co, trial_len, dt, 
                                    win_start=win_start, 
                                    win_stop=win_stop)#, align_peaks=True, align_win_start=0.0, align_win_stop=0.2)
        ...
        for j in range(n_co):
            ...
            #     min_theta = i * bin_theta
            #     max_theta = (i+1) * bin_theta
            #     x_binned = X[:,j*win_size:(j+1)*win_size][(theta > min_theta) & (theta <= max_theta)]
            ...
            axplot.set_title('Monkey %s Controller %d'%(dataset, j+1))
            inset.set_rticks([])
            inset.set_xticks([])
            inset.set_yticks([])
            ...
            fig.savefig('/home/macleanlab/peter/rockstar_ctrl_%d.png'%(j+1))
