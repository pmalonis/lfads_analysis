...
nbins = 12
fb_win_start = -0.15#0.00#-0.1#cfg['post_target_win_start']
fb_win_stop = 0.15#0.3#0.1#cfg['post_target_win_stop']
win_start = -0.35
win_stop = -0.05
...
if __name__=='__main__':
    ...
    for dset_idx, (dataset, param) in enumerate(zip(datasets, params)):
        ...
        plt.title('Monkey %s'%run_info[dataset]['name'])
    ...
    plt.savefig(os.path.join(os.path.dirname(__file__), '../figures/final_figures/rate_averages_correlation_with_maxima_means_all.svg'))
