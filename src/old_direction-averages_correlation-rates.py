...
if __name__=='__main__':
    for event_type in ['corrections', 'maxima']:
        all_means = []        
        fb_all_means = []
        for dset_idx, (dataset, param) in enumerate(zip(datasets, params)):
            ...
            # fb_peak_df_train = pd.read_pickle('../data/peaks/%s_%s_train.p'%(dataset, event_type))
            # fb_peak_df_test = pd.read_pickle('../data/peaks/%s_%s_test.p'%(dataset, event_type))
            ...
            # X,y = opt.get_inputs_to_model(peak_df, co, trial_len, dt, df=df, use_rates=True, rate_pcs=n_pcs,
            #                             reduce_neurons=False, win_start=win_start, 
            #                             win_stop=win_stop)

            # fb_X,fb_y = opt.get_inputs_to_model(fb_peak_df, co, trial_len, dt, df=df, use_rates=True, rate_pcs=n_pcs,
            #                                     reduce_neurons=False, win_start=fb_win_start, win_stop=fb_win_stop)
            X = np.random.randn(1000,150)
            y = np.random.randn(1000,2)
            fb_X = np.random.randn(1000,150)
            fb_y = np.random.randn(1000,2)
            ...
            plt.suptitle('Mean of Firing Rate Direction-Averages')
            #plt.tight_layout()
            ...
            # plt.figure(2)
            # plt.tight_layout(pad=2)
            # plt.suptitle('Max of Firing Rate Direction-Averages')
            # #plt.tight_layout()
            # plt.subplot(1,len(datasets),dset_idx+1)
        ...
        elif event_type == 'maxima':
            plt.xlabel('Initial Movement Direction-Mean (z-scored)')
            plt.ylabel('Max Speed Direction-Mean (z-scored)')
        ...
        plt.savefig('../figures/final_figures/rate_averages_firstmove_with_%s_means_5pcs.svg'%event_type)