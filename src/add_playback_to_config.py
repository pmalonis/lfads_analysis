...
if __name__=='__main__':
    ...
    server_path = 'macleanlab@205.208.22.226'
    config_path = os.path.join(os.path.dirname(__file__), '../config.yml')
    cfg = yaml.safe_load(open(config_path, 'r'))
    dsets = os.listdir(raw_root)
        ...
        for cond in conds:
            ...
            for param in params:
                for trial_type in ['valid', 'train']:
                    param_dict[trial_type] = lfads_root + 'single_' + cond + '/lfadsOutput/model_runs_active.h5_%s_posterior_sample_and_average'%trial_type
            dset_dict = {'raw':raw_path, param:param_dict}

            datasets['_'.join([dset, cond])] = dset_dict