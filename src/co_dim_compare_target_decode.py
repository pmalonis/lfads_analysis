...
for co_dim in co_dims:
    ...
    all_mean_scores.append(np.mean(mean_scores))
    all_std_scores.append(np.sqrt(np.sum(std_scores)))

all_mean_scores = np.array(all_mean_scores)
all_std_scores = np.array(all_std_scores)
plt.plot(co_dims, all_mean_scores)
plt.fill_between(co_dims, all_mean_scores-all_std_scores, 
                all_mean_scores+all_std_scores, alpha=0.2)
