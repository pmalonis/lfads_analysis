...
def fit_bell_shaped(T, C, N, epochs, loc, scale, optimizer, input_activation, 
    ...
    model.fit(x_train, [y_train, rate_train], epochs=epochs, verbose=0)
    predicted,rates = model(x_train)

    return predicted, rates, y_train, model, x_train
...
input_noise=0.00
predicted, rates, y_train, model, x_train = fit_bell_shaped(T, C, N, epochs, loc, scale, optimizer, input_activation, 
